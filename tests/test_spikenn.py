...
def calc_summ_stats_numpy(x, lags=[1, 10, 50, 100, 200, 500, 1000], norm=True):
    """
    Calculate and returns the following summary statistics for input trace: mean, variance, autocorrelation for input lags, crosscorrelation.
    Returns numpy matrix of size samples x number of summary statistics x neurons
    
    :param x: numpy matrix containing the rate trace of shape samples x timebins x neurons
    :param lags: list containing lags (in number of timebins) at which autocorrelation is to be calculated. Default is [1, 10, 50, 100, 200, 500, 1000]. 
    :param norm: boolean value. Set to True if autocorrelation and cross-correlation are to be normalised by mean and std. Default is True.
...
def prior_connectivity_matrix(
    ...
    """
        Sample connectivity matrices from MVN prior. 
        
        :param num_samples: number of samples required from prior. Default is 1.
        :param nE: number of excitatory neurons. Default is 2.
        :param nE: number of inhibitory neurons. Default is 3.
        :param W_vals: list of float values for mean of each quadrant (E-E, E-I, I-E, I-I) in connectivity matrix.
        :param std: standard deviation for covariance matrix.
        :param bandwidth: bandwidth for square exponential kernel used to construct covariance matrix.
    """
    ...
    # Draw samples
    W = np.random.normal(scale=std * cov, size=(num_samples, *cov.shape)) + mean

    return W.squeeze()


class SpikeNNnumpy:
    """Supralinear Spiking Neural - Hennequin et al 2018 - The Dynamical Regime of Sensory Cortex"""
    ...
    def __init__(
        ...
        # TODO: hard-coded for convenience -- need to make input for h more flexible, but more efficient than giving it to forward()
        self.h = (
    ...
    def forward(self, V=None, h=None, return_summ_stats=True, save_freq=1):
        ...
        :param V_init: numpy array containing initial voltage of length = neurons. If None, voltage is set to V0 
        :param h: external input to neurons of size timebins x neurons. If None, h is set to zero.
        :param return_summ_stats: boolean value. If True, returns summary statistics on rate traces. If False, returns voltage, rate and nosie traces.
        :param save_freq: number of update steps at which to save voltage, rate and noise traces. Default 1, implies values are saved after every update step.
...
def simulate_spikeNN(w: torch.Tensor) -> torch.Tensor:

    assert (
        w.numel() == 25
    ), "spikeNN needs exactly 25 parameter values and cannot handle batches."
    ss = SpikeNNnumpy(w.reshape(5, 5)).forward(return_summ_stats=True).reshape(1, 70)
...
class SpikeNNPrior:
    def __init__(
        ...
        self.dist = norm(loc=self.mean, scale=self.std * self.cov)

    def sample(self, sample_shape=torch.Size([])):

        if sample_shape == torch.Size([]):
            return torch.as_tensor(self.dist.rvs().reshape(25), dtype=torch.float32)
        else:
            num_samples = sample_shape[0]
            W = self.dist.rvs((num_samples, *self.cov.shape))
            return torch.as_tensor(W.reshape(num_samples, 25), dtype=torch.float32)

    def log_prob(self, value: torch.Tensor) -> torch.Tensor:

        num_values, value_shape = value.shape

        value = value.numpy().reshape(num_values, 5, 5)
...
def test_():
    ...
    assert Ws.shape == torch.Size([10, 25])

    log_probs = prior.log_prob(Ws)
    ...
    xs = []
    for th in theta:
        xs.append(simulate_spikeNN(th))

    print(torch.cat(xs).shape)
